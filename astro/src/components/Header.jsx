...
export default function Header({ phone, pathname }) {
  const menuItems = [
        ...
        <div className="container flex items-center justify-between py-4">
          ...
          <nav>
            <ul className="flex items-center space-x-2 md:space-x-12">
              {menuItems.map((item) => (
                <li key={item.href}>
                  <a href={item.href} className={clsx("menu-link", item.href === pathname && "active")}>
                    {item.label}
                  </a>
                </li>
              ))}
          ...
          <a href={`tel:${phone}`} className="btn flex items-center space-x-2 text-white bg-accent lg-down:p-0 lg-down:!bg-transparent">
            ...
            <span className="lg-down:hidden">{phone}</span>
          </a>
        </div>
      </header>
    </>

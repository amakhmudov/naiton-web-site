...
export default function SolutionsContent({ sections }: { sections: Section[] }) {
    ...
    <>
      <section className="container pt-12 pb-6 md-down:text-center">
        {sections.slice(0, 2).map((section, index) => (
          <SolutionSection key={section.id} {...section} showDivider={index < sections.slice(0, 2).length - 1} />
        ))}
      </section>
      ...
      <section className="container relative before:content-[''] before:absolute before:w-[calc(100%-2rem)] before:h-full before:top-0 before:bg-accent before:z-0 overflow-hidden">
        <div className="py-12 relative space-y-12 bg-no-repeat bg-top md-down:bg-contain nbswo-bg">
          ...
          <div className={clsx("relative md-down:px-8 text-white max-w-xl mx-auto space-y-3 overflow-hidden transition-all duration-1000", toggleDropdown || "max-h-80")}>
            ...
            <div className="absolute left-0 bottom-0 w-full pt-12 bg-linear-to-b via-accent from-transparent to-accent">
      ...
      <section className="container pt-12 pb-6 md-down:text-center">
        {sections.slice(2).map((section, index) => (
          <SolutionSection key={section.id} {...section} showDivider={index < sections.slice(2).length - 1} />
        ))}
      </section>
    </>
  );
